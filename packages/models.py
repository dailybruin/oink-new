...
class Package(models.Model):
 ...
 #  slug = models.SlugField(max_length=255, unique=True, help_text='The article slug (e.g. sports.mbb.oregon)')
   slug = models.CharField(max_length=255, unique=True, help_text='HELP',
   validators=[
       RegexValidator(
           # letters/numbers, separated by single dot or hyphen segments
           regex=r'^[a-z0-9]+(?:[.-][a-z0-9]+)*$',
           flags=re.I,
           message='Use only letters, numbers, dots, and hyphens (no spaces or special characters).'
       )
   ],
)
   ...
   def save(self, *args, **kwargs):
     ...
     #        self.google_drive_url = f"{root}/{safe}"
               safe = re.sub(r'[^a-zA-Z0-9._-]+', '-', self.slug).strip('.-')
               self.google_drive_url = f"{root}/{safe}"
