...
def google_callback(request):
    ...
    if not code:
        return render(request, 'app/error.html', {'error': 'No code from Google'})
    ...
    if 'error' in token_resp:
        ...
        return render(request, 'app/error.html', {'error': token_resp})
    ...
    if not email or not email.endswith('@' + settings.EMAIL_DOMAIN):
        return render(request, 'app/error.html', {'error': f'Must sign in with a {settings.EMAIL_DOMAIN} email'})
